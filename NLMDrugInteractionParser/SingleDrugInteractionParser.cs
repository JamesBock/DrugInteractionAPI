    ...
    public class SingleDrugInteractionParser : IDrugInteractionParser
    {
        public List<MedicationInteractionPair> ParseDrugInteractions(string jstring)
        {
            ...
            catch (NullReferenceException)
            {
                ...
                interactionPairCount = j["interactionTypeGroup"][0]["interactionType"].Children()["interactionPair"].ToList().Count();
            for (int f = 0; f < interactionPairCount; f++)
            {
                interactionConceptCount = j["interactionTypeGroup"][0]["interactionType"][f]["interactionPair"].Children()["interactionConcept"].ToList().Count();

                for (int i = 0; i < interactionConceptCount; i++)
                {
                    var interaction = new MedicationInteractionPair() { InteractionId = Guid.NewGuid() };

                    interaction.Comment = j["interactionTypeGroup"][0]["interactionType"][f]["comment"].ToString();
                    interaction.MedicationPair = (new MedicationInteractionPair.MedicationViewModel() { DisplayName = j["interactionTypeGroup"][0]["interactionType"][f]["interactionPair"][i]["interactionConcept"][0]["minConceptItem"]["name"].ToString(), RxCui = j["interactionTypeGroup"][0]["interactionType"][f]["interactionPair"][i]["interactionConcept"][0]["minConceptItem"]["rxcui"].ToString() }, new MedicationInteractionPair.MedicationViewModel() { DisplayName = j["interactionTypeGroup"][0]["interactionType"][f]["interactionPair"][i]["interactionConcept"][1]["minConceptItem"]["name"].ToString(), RxCui = j["interactionTypeGroup"][0]["interactionType"][f]["interactionPair"][i]["interactionConcept"][1]["minConceptItem"]["rxcui"].ToString() });
                        ...
                        detail.Description = j["interactionTypeGroup"][0]["interactionType"][f]["interactionPair"][i]["description"].ToString();

                        detail.Severity = j["interactionTypeGroup"][0]["interactionType"][f]["interactionPair"][i]["severity"].ToString();

                        //if the source is the JAMIA article, the uri is of the article.
                        detail.LinkTupList = new List<(string, Uri)>(j["interactionTypeGroup"][0]["interactionType"][f]["interactionPair"][i]["interactionConcept"].Children()["minConceptItem"]["name"].ToList()
                                                        .Select(x => x.ToString().ToUpper())
                                                       .Zip(j["interactionTypeGroup"][0]["interactionType"][f]["interactionPair"][i]["interactionConcept"].Children()["sourceConceptItem"]["url"].ToList(), (first, second) => (first,
        ...
        public Task<List<MedicationInteractionPair>> ParseDrugInteractionsAsync(string jstring)
        {
            return Task.Run(() =>
            {
                return ParseDrugInteractions(jstring);
            });
        }
    }
